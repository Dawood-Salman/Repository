...
import {
  ...
  Upload as UploadIcon, Download as DownloadIcon, Description as DescriptionIcon,
...
const Teachers = () => {
  ...
  const handleBulkUpload = async () => {
    ...
    setUploading(true);
    try {
      const { data } = await api.bulkUploadTeachers(companyId, selectedFile);
    ...
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Button variant="outlined" startIcon={<UploadIcon />} onClick={handleBulkUploadOpen} sx={{ fontWeight: 600, color: '#FF6B35', borderColor: '#FF6B35' }}>
            Bulk Upload
          </Button>
                ...
                <>
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth>
                      <InputLabel>Enable Login Access</InputLabel>
                      <Select
                        value={formData.loginEnabled}
                        label="Enable Login Access"
                        onChange={e => handleChange('loginEnabled', e.target.value)}
                      >
                        <MenuItem value={false}>No Login</MenuItem>
                        <MenuItem value={true}>Yes - Create Login (Default password: 123456789)</MenuItem>
                      </Select>
                    </FormControl>
                  </Grid>
              ...
              <Box sx={{ 
                ...
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Upload an Excel file with teacher data. Required fields: name, contact. 
                    Optional fields: email, address, specialization, salaryType, annualSalary, fixedSalary, accountName, accountNumber, bankName, ifscCode, loginEnabled, openingBalance, openingBalanceDate
                  </Typography>
                </Box>
              </Box>
            </Grid>
                    ...
                    <Box sx={{ mt: 2 }}>
                      ...
                      <Box sx={{ maxHeight: 200, overflow: 'auto', mt: 1 }}>
                        {uploadResults.results.duplicates.map((item, index) => (
                          <Typography key={index} variant="caption" color="text.secondary" display="block">
                            Row {item.row}: {item.error} (Existing ID: {item.existingTeacherId})
                          </Typography>
                        ))}
                      </Box>
                    </Box>
