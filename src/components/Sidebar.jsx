...
import BeinnovoLogo from './BeinnovoLogo';
  ...
  const menuItems = React.useMemo(() => {
    ...
    if (isAdmin) {
      items.push(
        ...
        { text: 'Notifications', icon: <NotificationsIcon />, path: `/company/${companyId}/notifications`, roles: ['superadmin', 'admin'] },
      );
    }
  ...
  const drawerContent = (
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        minHeight: 64,
        borderBottom: `1px solid ${alpha('#1055B8', 0.2)}`,
      }}>
        {isCollapsed ? (
          <BeinnovoLogo size={34} />
        ) : (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
            <BeinnovoLogo size={34} />
            <Box>
              <Box sx={{ display: 'flex', alignItems: 'baseline' }}>
                <Typography sx={{ color: 'white', fontWeight: 900, fontSize: '1rem', lineHeight: 1, letterSpacing: '-0.02em' }}>Bein</Typography>
                <Typography sx={{ color: '#42A5F5', fontWeight: 900, fontSize: '1rem', lineHeight: 1, letterSpacing: '-0.02em' }}>novo</Typography>
              </Box>
              <Typography sx={{ color: alpha('#FFF', 0.35), fontWeight: 700, fontSize: '0.55rem', textTransform: 'uppercase', letterSpacing: '0.12em', mt: 0.2 }}>
                EIMS Platform
              </Typography>
            </Box>
          </Box>
        ...
        {!isCollapsed && (
          <IconButton 
            ...
            sx={{ color: alpha('#FFF', 0.4), '&:hover': { bgcolor: alpha('#FFF', 0.08) }, ml: 0.5 }}
          >
        ...
        {isCollapsed && (
          <IconButton 
            size="small"
            onClick={() => setIsCollapsed(!isCollapsed)}
            sx={{ position: 'absolute', bottom: 16, left: '50%', transform: 'translateX(-50%)', color: alpha('#FFF', 0.4), '&:hover': { bgcolor: alpha('#FFF', 0.08) } }}
          >
